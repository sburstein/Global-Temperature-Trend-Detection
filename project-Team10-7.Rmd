...
Climate change is a significant issue that has also become a hot-button political topic in recent years because of the failure of certain individuals, political parties, and corporations to take action. 
...
To evaluate this, we will conduct two general hypothesis tests:
...
Our hypotheses for this question are as follows:
$H_o$: The global average earth surface temperature changed at the same rate from 1980-2013 as it did from 1900-1930.

$H_a$: The global average earth surface temperature changed at a greater rate from 1980-2013 than it did from 1900-1930.
...
m_int <- lm(comp ~ year + post1980 + year*post1980, data=comparison_to_20th)
tidy(m_int)
```
...
```{r q-2-vis}
ggplot(data=comparison_to_20th, aes(x=year, y=comp, color=post1980)) +
  geom_point() +
