...
m_int_2000 <- lm(avg ~ year + post2000 + year*post2000, data=annual_avg)
tidy(m_int_2000)%>%
  select(term, estimate)

m_int <- lm(avg ~ year + time_period + year*time_period, data=annual_avg)
tidy(m_int)
...
Post-2000:
$$\widehat {TempDiff} = -63.65442 + 0.0365 year $$



```{r q-2-viz, echo = FALSE}
ggplot(data=annual_avg, aes(x=year, y=avg, color=time_period)) +
  geom_point() +
...
slopes <- tibble::rowid_to_column(slopes, "year")
slopes <- slopes %>%
  mutate(yearstart = 1895 + 5 * year)

ggplot(data=slopes, aes(x=slopes)) +
  geom_histogram(bins=50) +
  labs(title="Distribution of rates of temp. change for each five-year period",
       subtitle="From 1900-2010",
...
ggplot(data=boot_dist_slope, aes(x=boot_dist_slope)) +
  ...
  geom_vline(xintercept = post2000slope, 
             lwd = 1, color = "red")
  labs(title=
         "Simulated null distribution of slopes of avg. temperature change")
