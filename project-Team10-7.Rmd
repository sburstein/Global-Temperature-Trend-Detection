...
To evaluate this main question, we will conduct comprehensive hypothesis tests on four 
sub-questions.
...
For this question, we will create a dataframe storing the observed average rate 
of change (slope) for the linear model generated for every five-year period, and 
test whether the mean value for slope after 1980 is greater than the mean slope 
before 1980 by a statistically significant margin. 
...
```{r visualization-by-season,warning=FALSE, echo = FALSE,fig.dim = c(7, 3),fig.align="center"}
...
```{r annual-avg-viz, message = FALSE, echo = FALSE,fig.dim = c(7,3),fig.align="center"}
annual_avg <- df %>%
  group_by(year) %>%
...
```{r facet-season, echo = FALSE, fig.dim = c(7,3),fig.align="center"}
ggplot(data=df, aes(x=dt, y=LandAverageTemperature)) +
...
```{r highs-lows-viz, echo = FALSE, fig.dim = c(7,3), fig.align="center"}
...
```{r viz-by-country, echo = FALSE, fig.dim = c(8,3),fig.align="center"}
legend_mod <- function() {
  theme(legend.position="right",
...
```{r compare-to-20th-sim-viz, echo = FALSE, fig.dim = c(7,3),fig.align="center"}
...
In this hypothesis test, the test statistic is represented by the slope that 
...
```{r 1980-lm-viz, message = FALSE, echo = FALSE, fig.dim = c(7,3), fig.align="center"}
...
``` {r slopes-viz, echo = FALSE, fig.dim = c(7,3),fig.align="center"}
...
temperature increase change after 1980",
       ...
       x = 
         "Calculated slope (rate of change in avg. earth surface temperature)",
       y = "Count") + 
  theme_economist_mod() +
  theme(plot.subtitle = element_text(size = 14, hjust = 0))
```

From the above simulation-based test, we attain a p-value of 0. We can then
...
To support this, we also conducted the same hypothesis test with 1940 as a demarcation, 
...
For this test, we calculated a p-value of 0.0918, indicating a higher
probability that the difference of pre-1940 and post-1940 slopes observed in the 
