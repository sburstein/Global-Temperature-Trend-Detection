...
Our hypotheses for this question are as follows:
$H_o$: The rate at which the average global land temperature increased after 
1980 is not greater than the rate of temperature increase starting from 1900
based on their respective linear models.

$H_a$: The rate at which the average global land temperature increased after 
...
overall 20th century average temperature (positive or negative) after 1980 than 
was present before 1980? *
...
North America -  
$H_{o}$: The change in mean temperature from before 1950 to post-2010 in North 
America is equal to the change in temperature observed in the rest of the world.  

$H_{a}$: The change in mean temperature from before 1950 to post-2010 in North 
...
for (i in 1:5000) {
  ...
  pre_boot_mean <- pre1980_net %>%
    ...
    summarize(mean(differences)) %>%
    pull()
  net_boot_dist[i] <- abs(post_boot_mean) - abs(pre_boot_mean)
}
...
diff_bootstrap_interval
```
Our 99% confidence interval for this hypothesis test is (-0.04495, 0.15154).
...
post1980_net_diff_mean <- post1980_net %>%
  summarize(abs(mean(differences))) %>%
  pull()
...
performed here are both greater than $\alpha = 0.05$, so we fail to reject the 
null hypothesis. In this case, the p-value represents the likelihood of 
obtaining results where the net fluctuation from year to year in 1980 has a 
difference with respect to the net fluctuations before 1980 as great as observed
or greater. In context of our question, this means we may not have sufficient 
evidence of an increase in overall temperature changes from year to year in more 
recent decades compared to earlier in the 20th century.
...
equal to the global change.

$H_a: \mu_n \neq \mu_g$
The change in mean temperature from before 1950 to post-2010 in North America is 
not equal to the global change.
...
t.test(change ~ is_northamerica,
       ...
       conf.level = 0.95)
```
From these two hypothesis tests, we fail to reject the null hypothesis that 
North America has a different change in mean temperature from 1950-2010 than the
rest of the world. The p-value was 0.268, indicating a .268 probability of 
observing results in which North America had a difference of overall temperature
change at least as extreme as shown in this data set.

However, we constructed a similar t-test for Europe, and when comparing the 
confidence intervals for the two, comparing Europe to the rest of the world 
produces a much smaller confidence interval (-.4124, .4133), versus that for 
North America: (1.2704, 2.0948). Additionally, the p-value for Europe's t-test 
was 0.9982, indicating a higher likelihood of Europe having the same true mean 
...
changesbycountry$country[12] <- "Ivory Coast"
...
customColorGrad <-  colourPalette <- RColorBrewer::brewer.pal(6,'YlOrRd')
#https://www.mathworks.com/matlabcentral/mlc-downloads/downloads/e5a6dcde-4a80-11e4-9553-005056977bd0/2639c6af-d83e-4d5b-8d3c-bdf046fb5048/images/screenshot.png

mapCountryData(geo_country_df, nameColumnToPlot="change",
               mapTitle= "Temperature Change",
               colourPalette= customColorGrad,
               oceanCol="lightblue",
...
century. Our second hypothesis test found insufficient evidence of a 
statistically significant increase in the average degree of temperature change 
from before 1980 to after 1980, but did find a statistically significant 
increase in average degree of temperature change after 2000. While 
the data set had limited values after 2000, it still shows evidence that
global warming is becoming more severe. This could be due to increased
greenhouse gas emissions in recent years. Our third hypothesis test found that 
we may not have sufficient evidence of an increase in overall temperature 
changes from year to year in more recent decades compared to earlier in the 20th 
century. However, temperature is still increasing. The last hypothesis test 
found that both Europe and the United States of America do not significantly 
differ from the mean change in temperature globally, but that the U.S. is more 
variable with their temperature changes. This provides evidence towards the 
effectiveness of initiatives in the European Union against climate change (such
as the Paris Agreement) that the USA refrained from participating in.

While our findings supported the existence of climate change, it is important
to critique our data and methods. The dataset that we found included a wide 
variety of cases, but was limited. For example, there were less European and
South American cities included than Asian cities. This limits our ability to
draw conclusions about Europe and South America. Another limitation was the lack
of data in earlier years. The dataset included many NA values in the earliest 
years, which is why we had to focus our analysis on the last hundred years. We 
also have to take into account the fact that temperature collecting 
equipment has improved over time, so more recent observations are more 
accurate than older observations.

Some limitations of our methods were,

If we were able to restart the project, 
